...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        if len(position) in [0, 1]:
            return len(position)

        mp = {}
        ptr = 0
        while ptr < len(position):
            mp[position[ptr]] = speed[ptr]
            ptr += 1
        position.sort(reverse=True)
        stk = []
        for pos in position:
            diff = target - pos
            time_diff = diff / mp[pos]
            if not stk or (stk and stk[len(stk) - 1] < time_diff):
                stk.append(time_diff)
        return len(stk)
